...
VSlices exists because several software engineering problems tend to appear together:

| Problem | What it causes |
| --- | --- |
| [Knowledge connection gap](#a-knowledge-connection-gap) | People learn concepts separately, but struggle to connect them in real systems. |
| [Continuity problem](#a-continuity-problem) | Discovery, documentation, architecture, and implementation drift apart over time. |
| [AI context problem](#an-ai-context-problem) | AI can generate outputs quickly, but weak context makes those outputs rely on assumptions. |
| [Progressive complexity problem](#a-progressive-complexity-problem) | Teams either add structure too early or avoid it for too long. |
| [Hidden behavior problem](#a-hidden-behavior-problem) | Important business behavior becomes implicit, scattered, or hard to see. |
...
VSlices exists because software engineering is not only a sequence of brand-new inventions. It is also a long history of discoveries, rediscoveries, refinements, and connections between ideas that were already there.
...
The result may not always be visible at first, but over time, the system becomes harder to explain, harder to change, and harder to trust.
...
This changes how software is produced, but it does not eliminate the need for engineering. In many cases, it increases it because AI systems generate outputs based on:

- their training
- the information available in the current context
- the constraints explicitly provided
...
The challenge is not that AI generates code, but ensuring that the generated code reflects the actual needs, constraints, and intentions of the system being built.
...
| --- | --- |
| Too much architecture too early | Teams start with layers, patterns, abstractions, conventions, and infrastructure decisions before the domain shows they are necessary. |
| Too little structure for too long | Teams start simple, but as the system grows, behavior becomes implicit, boundaries become blurry, and changes become harder to reason about. |
...
We cannot see the future, but we can reason about expected usage, likely evolution, operational pressure, and how the system may impact different parts of an organization in the short and long term.
